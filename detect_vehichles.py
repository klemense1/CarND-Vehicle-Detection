...
if __name__ == "__main__":
    ...
    perm_cars = np.arange(len(cars))
    perm_notcars = np.arange(len(notcars))
    np.random.shuffle(perm_cars)
    np.random.shuffle(perm_notcars)
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X_train_all, y_train_all, test_size=0.2, random_state=rand_state)
