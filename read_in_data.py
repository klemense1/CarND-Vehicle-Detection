...
def train_test_split_procedual(listin):
    length = len(listin)
    idx_train_end = int(length*0.9)

    training_data = listin[:idx_train_end]
    test_data = listin[idx_train_end:]
...
def read_in_seperately(path_cars, path_notcars):
    ...
    for pix_dir_cars in pix_dir_list_cars:
        tra_c, test_c = train_test_split_procedual(pix_dir_cars)
    ...
    for pix_dir_no in pix_dir_list_no:
        tra_n, test_n = train_test_split_procedual(pix_dir_no)
        train_data_no = train_data_no + tra_n
        test_data_no = test_data_no + test_n

    return train_data_cars, test_data_cars, train_data_no, test_data_no
...
if __name__ == "__main__":
    ...
    train_data_cars, test_data_cars, train_data_no, test_data_no = read_in_seperately(path_cars, path_notcars)
